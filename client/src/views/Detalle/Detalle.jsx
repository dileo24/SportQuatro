...
export default function Detalle() {
 ...
 const handleEdit = () => {
  ...
  const handleSave = async () => {
    try {
      const response = await axios.put(`http://localhost:3001/autos/${id}`, editedAuto);
      if (response.data.status === 200) {
        setAuto(editedAuto);
        setIsEditing(false);
            ...
            <CardContent>
              {isEditing ? (
                <TextField
                  ...
                  onChange={(e) => setEditedAuto({ ...editedAuto, modelo: e.target.value })}
                  fullWidth
                  margin="normal"
                />
              ) : (
                ...
                {isEditing ? (
                  <Box display="flex" alignItems="center">
                    <TextField
                      ...
                      onChange={(e) => setEditedAuto({ ...editedAuto, precio: e.target.value })}
                      fullWidth
                      margin="normal"
                    />
                    <select
                  ...
                  <Box sx={{ fontSize: '1.5rem', fontWeight: 'bold', mb: 2, textAlign: 'center' }}>
                    {moneda} {auto.precio}
                  </Box>
                )}
